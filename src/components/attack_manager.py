...
class Attack_Manager:
    ...
    def create_black_holes(
        self,
        number_of_black_holes: int,
        swap_prob: int | float,
        targets_per_black_hole: int,
    ) -> None:
        """
        ...
        if swap_prob < 0 or swap_prob > 1:
            log.warning(
                f"No black hole was created. The parameter swap_prob is invalid: {swap_prob}"
            )
            return
        ...
        if targets_per_black_hole < 1:
            self._create_black_holes_no_targets(
                number_of_black_holes=number_of_black_holes, swap_prob=swap_prob
            )
        else:
            self._create_black_holes_with_targets(
                number_of_black_holes=number_of_black_holes,
                swap_prob=swap_prob,
                targets_per_black_hole=targets_per_black_hole,
            )
        ...
        for bh in self.network.black_holes.values():
            log.debug(
                f"{bh.name} is a black hole with targets: {bh._black_hole_targets}"
            )
    ...
    def _create_black_holes_no_targets(
        self, number_of_black_holes: int, swap_prob: int | float
    ) -> None:
        """
        ...
        if number_of_black_holes < 1:
            log.warning(
                f"No black hole was created. The parameter number_of_black_holes is invalid: {number_of_black_holes}"
            )
            return

        if number_of_black_holes >= len(self.network.normal_nodes):
            log.warning(
                f"No black hole was created. The parameter number_of_black_holes is invalid: {number_of_black_holes} > number of normal nodes"
            )
            return
    ...
    def _create_black_holes_with_targets(
        self,
        number_of_black_holes: int,
        swap_prob: int | float,
        targets_per_black_hole: int,
        ...
        if targets_per_black_hole <= 0:
            log.warning(
                f"No black hole was created. The parameter targets_per_black_hole is invalid: {targets_per_black_hole}"
            )
            return
        elif len(self.network.normal_nodes) <= targets_per_black_hole:
            log.warning(
                f"No black hole was created. The parameter targets_per_black_hole is invalid: {targets_per_black_hole} >= number of normal nodes"
            )
            return
        ...
        while counter > 0:
            ...
            while targets_counter > 0:
                ...
                tmp_normal_node: QuantumRepeater = self.network.nodes[
                    tmp_normal_node_id
                ]
        ...
        log.debug(
            f"{number_of_black_holes} were created with {targets_per_black_hole} targets. Black Holes: {self.network.black_holes.keys()}"
        )
