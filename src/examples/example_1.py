...
class SimpleManager:
    ...
    def update(
        self, protocol: EntanglementProtocol, memory: Memory, state: str
    ) -> None:
        if state == RAW:
            self.raw_counter += 1
            memory.reset()
...
class TopologyGen:
    ...
    def _create_nodes(self, number_of_nodes: int) -> dict[int, EntangleGenNode]:
        ...
        for c in range(0, number_of_nodes):
            tmp_node: EntangleGenNode = EntangleGenNode(
                name=f"node{c}", timeline=self.network.timeline
            )
            tmp_node.set_seed(c)
            nodes[c] = tmp_node
    ...
    def _create_BSMNode(
        self, nodeA_id: int, nodeB_id: int, seed_counter: int, edge: tuple[int, int]
    ) -> BSMNode:
    ...
    def _connect_quantum_channels(
        ...
        qc1 = QuantumChannel(
            name=f"qc{seed_counter}",
            timeline=self.network.timeline,
            attenuation=qc_attenuation,
            distance=qc_distance,
        )
        qc2 = QuantumChannel(
            name=f"qc{seed_counter+1}",
            timeline=self.network.timeline,
            attenuation=qc_attenuation,
            distance=qc_distance,
        )
