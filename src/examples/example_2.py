    ...
    def create_protocolB(self, memory_position: str) -> None:
        ...
        if memory_position == LEFT:
            ...
            protocol: EntanglementSwappingB = EntanglementSwappingB(
                self.owner, f"{self.owner.name}.ESB", left_memo
            )
            self.owner.protocols.append(protocol)
            ...
            right_memo: Memory = self.owner.components[self.owner.right_memo_name]
            protocol: EntanglementSwappingB = EntanglementSwappingB(
                self.owner, f"{self.owner.name}.ESB", right_memo
            )
            self.owner.protocols.append(protocol)
...
def entangle_memory(
    tl: Timeline, memo1: Memory, memo2: Memory, fidelity: float
) -> None:
...
def pair_protocol(
    node1: QuantumRepeater, node2: QuantumRepeater, node_mid: QuantumRepeater
) -> None:
    ...
    p1.set_others(
        pmid.name, node_mid.name, [node_mid.left_memo_name, node_mid.right_memo_name]
    )
    p2.set_others(
        pmid.name, node_mid.name, [node_mid.left_memo_name, node_mid.right_memo_name]
    )
