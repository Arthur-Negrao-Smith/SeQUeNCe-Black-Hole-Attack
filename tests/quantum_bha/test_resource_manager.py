...
class Test_Repeater_Manager:
    ...
    def test_get_memory(self) -> None:
        ...
        node: QuantumRepeater = QuantumRepeater(
            name="test_node", timeline=Timeline(), swap_prob=1.0
        )

        assert (
            node.resource_manager.get_memory(Directions.LEFT).name
            == node.left_memo_name
        )
        assert (
            node.resource_manager.get_memory(Directions.RIGHT).name
            == node.right_memo_name
        )
    ...
    def test_creating_entanglement_protocol(self, network: Network) -> None:
        ...
        node: QuantumRepeater = network.nodes[0]
        node.resource_manager.create_entanglement_protocol(
            Directions.LEFT, "middle_node", "other_node"
        )
        assert isinstance(node.get_protocol(), EntanglementGenerationA)

        node.remove_used_protocol()
        node.resource_manager.create_entanglement_protocol(
            Directions.RIGHT, "middle_node", "other_node"
        )
        assert isinstance(node.get_protocol(), EntanglementGenerationA)
    ...
    def test_turn_black_hole(self, network: Network) -> None:
        ...
        nodeA.resource_manager._turn_black_hole(
            new_swap_prob=self.new_swap_prob, targets=None
        )
        assert nodeA._is_black_hole == True
        assert nodeA._swap_prob == self.new_swap_prob
        assert nodeA._black_hole_targets is None

        nodeB.resource_manager._turn_black_hole(
            new_swap_prob=self.new_swap_prob,
            targets={nodeA.name: self.new_swap_prob_to_target},
        )
        ...
        nodeC.resource_manager._turn_black_hole(
            new_swap_prob=self.new_swap_prob, targets={nodeB.name: -1}
        )
    ...
    def test_turn_normal_node(self, network: Network) -> None:
        ...
        nodeA.resource_manager._turn_normal_node(
            new_swap_prob=ENTANGLEMENT_SWAPPING_PROB
        )
        ...
        nodeB.resource_manager._turn_normal_node(
            new_swap_prob=ENTANGLEMENT_SWAPPING_PROB
        )
        ...
        nodeC.resource_manager._turn_normal_node(
            new_swap_prob=ENTANGLEMENT_SWAPPING_PROB
        )
